...
export async function buildingWorkspace(buildingId:string){const state=await workspace();const building=state.buildings.find(b=>b.id===buildingId);if(!building)throw new NotFoundError();const user=await requireUser();const {data:membership,error}=await user.client.rpc('my_building_role',{p_building:buildingId});checkDb(error);const {data:permissions,error:e}=await user.client.from('role_permissions').select('permission').eq('role',membership);checkDb(e);const unread=await user.client.from('notifications').select('id',{count:'exact',head:true}).eq('building_id',buildingId).eq('state','new');checkDb(unread.error);return {...state,building,unreadUpdates:unread.count??0,permissions:z.array(z.object({permission:z.string()})).parse(permissions).map(p=>p.permission)};}
