...
export async function mutateAction(raw:unknown):Promise<{ok:true;id?:string;url?:string}|{ok:false;error:string}>{
 try{
  ...
  switch(input.operation){
   ...
   case 'bylaw.register':{const v=values.register.parse(input.values);result=await db.rpc('record_registered_bylaw',{p_id:requiredId(),p_filing:v.filing,p_effective:v.effective});break;}
